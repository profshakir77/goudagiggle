...
const PRODUCTS = [
  ...
  },
  {
    name: "Small Lunchbox",
    description:
      "Our Small Lunchbox is perfect for on-the-move snacking, office lunches, and individual gifts. Each box is packed with rolled premium meats, cubed artisan cheeses, fresh grapes, crunchy crackers, and a sweet treat — all neatly packaged and ready to enjoy anywhere in the Capital Region.",
    price: "20.00",
    category: "Grab & Go",
    serves: "1-2 guests",
  ...
  },
  {
    name: "Large Lunchbox",
    description:
      "Our Large Lunchbox is a hearty spread for those who love variety. Loaded with premium cured meats, a selection of artisan cheeses, seasonal fruits, nuts, olives, gourmet crackers, and house-made jam — a perfect grab-and-go gift or personal treat from Gouda Giggles in Latham, NY.",
    price: "36.00",
    category: "Grab & Go",
    serves: "2-3 guests",
    imageUrl: "/images/cat-grab-and-go.webp",
...
export async function seedProducts() {
  try {
    ...
    const expectedNames = new Set(PRODUCTS.map((p) => p.name));
    const hasCorrectData =
      existing.length === PRODUCTS.length &&
      existing.every((p) => expectedNames.has(p.name));
